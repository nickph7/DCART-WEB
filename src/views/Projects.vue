      ...
      <li class="" v-for="project in projects" :key="project.id">
        <router-link :to="'/' + project.id">
          <figure class="relative rounded-theme border border-solide border-black overflow-hidden" style="padding-bottom: 90%;">
            <div class="absolute inset-x-0 h-5 w-full bg-white border-b border-solid border-black z-10">
              <figcaption class="font-display text-center text-xs">{{ transformTitle(project.content.title) }}</figcaption>
            </div>
